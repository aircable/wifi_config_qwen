...
static bool led_state = false;  // track LED on/off state
...
static esp_err_t on_web_socket_url(httpd_req_t *req){
    ...
    if (req->method == HTTP_GET) {
        // send current LED state on new connection
        char msg[64];
        int len = snprintf(msg, sizeof(msg), "{\"type\":\"state\",\"state\":%s}", led_state ? "true" : "false");
        if (len > 0) {
            send_ws_message(msg);
        }
        return ESP_OK;
    ...
    // receive message and handle toggle request
    ...
    // parse JSON payload
    cJSON *root = cJSON_ParseWithLength((char *)ws_pkt.payload, ws_pkt.len);
    if (root) {
        cJSON *type = cJSON_GetObjectItem(root, "type");
        if (cJSON_IsString(type) && strcmp(type->valuestring, "toggle") == 0) {
            led_state = !led_state;
            toggle_led(led_state);
            char resp[64];
            int rlen = snprintf(resp, sizeof(resp), "{\"type\":\"state\",\"state\":%s}", led_state ? "true" : "false");
            if (rlen > 0) {
